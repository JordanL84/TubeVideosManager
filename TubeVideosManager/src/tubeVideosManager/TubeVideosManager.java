...
public class TubeVideosManager implements TubeVideosManagerInt {
	 ...
	 * validity of parameters. If an exception takes place while creating a video,
	...
	public ArrayList<Video> getAllVideosInDB() {
		ArrayList<Video> copyList = new ArrayList<Video>();
		for (Video video : videoDatabase) {
			copyList.add(new Video(video));
		}
		return copyList;
	}
	...
	public boolean addPlaylist(String playlistName) {
		if (playlistName == null || playlistName.isEmpty())
			throw new IllegalArgumentException("IVALID");
	...
	public void clearDatabase() {
		while (!(videoDatabase.isEmpty()))
			videoDatabase.remove(0);
		...
		for (Video video : videoDatabase) {
			if (title == null || video.getTitle().equals(title)) {
				if (maximumDurationInMinutes < 0 || video.getDurationInMinutes() == maximumDurationInMinutes) {
					if (genre == null || video.getGenre() == genre)
						newPlaylist.addToPlaylist(video.getTitle());
				}
			}
		}
		return newPlaylist;	
